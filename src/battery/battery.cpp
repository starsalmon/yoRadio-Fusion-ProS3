...
bool battery_alert_active() { return false; }
...
static BatterySample sampleBatteryClamped() {
  ...
  if (!isfinite(pct) || pct < -1.0f || pct > 101.0f || !isfinite(v) || v < 0.0f ||
      v > 5.0f || !isfinite(rate) || rate < -1000.0f || rate > 1000.0f) {
    delay(60);
...
static bool usbPresentNow() { return usbSenseReady && lastUsbPresent; }
...
static bool sampleLooksLikeStaleFull(const BatterySample& s) {
  if (s.v < 3.0f || s.v > 4.35f) return false;
...
static void max17048_setSocAlertPct(uint8_t pct) {
  // CONFIG.ATHD: alert_pct = 32 - ATHD
  if (pct < 1) pct = 1;
  if (pct > 32) pct = 32;
  const uint8_t athd = (uint8_t)(32u - pct);

  uint16_t cfg = 0;
  if (!max17048_read16(MAX1704X_CONFIG_REG, cfg)) return;
  cfg = (uint16_t)((cfg & 0xFFE0u) | (uint16_t)(athd & 0x1Fu));
  (void)max17048_write16(MAX1704X_CONFIG_REG, cfg);
}
...
#ifdef BATTERY_INT
  if (BATTERY_INT != 255) {
    pinMode(BATTERY_INT, INPUT_PULLUP);
    alertReady = true;
  }
#endif

#ifndef BATTERY_ALERT_PCT
  #define BATTERY_ALERT_PCT 5
#endif
  max17048_setSocAlertPct((uint8_t)BATTERY_ALERT_PCT);
  (void)maxlipo.clearAlertFlag(MAX1704X_ALERTFLAG_SOC_LOW);
...
void battery_update() {
  ...
  if (alertReady) {
#ifdef BATTERY_INT
    lastAlertActive = (digitalRead(BATTERY_INT) == LOW);
    if (lastAlertActive) {
      (void)maxlipo.clearAlertFlag(MAX1704X_ALERTFLAG_SOC_LOW);
    }
#endif
...
float battery_get_percent() { return lastPercent; }
bool battery_is_ready() { return batteryReady; }
bool battery_is_present() { return batteryReady && !voltageDefinitelyNoBattery(lastVoltage); }
float battery_get_voltage() { return lastVoltage; }
float battery_get_charge_rate() { return lastChargeRate; }
...
bool battery_alert_active() { return batteryReady && alertReady && lastAlertActive; }
