...
Note: some upstream text references `yoRadio/src/...`; in this fork the paths are `src/...`.
...
- **Location:** `MyNetwork::begin()`, `src/core/network.cpp`.

### `on_start_play()`
- **Description:** Triggered when playback starts.  
- **Location:**  
  - `Player::_play(uint16_t stationId)`, `src/core/player.cpp`.  
  - `Player::browseUrl()`, `src/core/player.cpp`.

### `on_stop_play()`
- **Description:** Triggered when playback stops.  
- **Location:** `Player::_stop(bool alreadyStopped)`, `src/core/player.cpp`.

### `on_track_change()`
- **Description:** Triggered when the current track changes.  
- **Location:** `Display::_title()`, `src/core/display.cpp`.

### `on_station_change()`
- **Description:** Triggered when the current station changes.  
- **Location:** `Player::loop()`, `src/core/player.cpp`.

### `on_display_queue(requestParams_t &request, bool &result)`
- **Description:** Triggered when a command is dequeued for the display.  
- **Parameters:**  
  - `request`: Reference to a `requestParams_t` structure (defined in `src/core/common.h`).  
  - `result`: Set to `false` to stop queue processing or `true` to continue.  
- **Location:** `Display::loop()`, `src/core/display.cpp`.

### `on_display_player()`
- **Description:** Triggered when the player UI is displayed.  
- **Location:**  
  - `Display::_start()`, `display.cpp`.  
  - `Display::_swichMode(displayMode_e newmode)`, `src/core/display.cpp`.

### `on_ticker()`
- **Description:** Triggered once every second.  
- **Location:** `ticks()`, `src/core/network.cpp`.
...
- **Location:** `onBtnClick(int id)`, `src/core/controls.cpp`.
