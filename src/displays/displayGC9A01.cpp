...
static Arduino_DataBus* bus = new Arduino_ESP32SPI(
  TFT_DC, TFT_CS, TFT_SCLK, TFT_MOSI, TFT_MISO, HSPI
);
#else
static Arduino_DataBus* bus = new Arduino_ESP32SPI(
  TFT_DC, TFT_CS, TFT_SCLK, TFT_MOSI, TFT_MISO, FSPI
);
#endif
...
yoDisplay::yoDisplay()
: Adafruit_GFX(240, 240), _gfx(make_gc9a01()) {}
...
void yoDisplay::drawRGBBitmap(int16_t x, int16_t y, const uint16_t *bitmap, int16_t w, int16_t h) {
  sdog.takeMutex();
  if (_gfx) {
